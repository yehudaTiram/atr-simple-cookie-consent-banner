...
class ATR_Simple_Cookie_Consent_Banner_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( dirname( __FILE__ ) ) . 'atr-scb.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( dirname( __FILE__ ) ) . 'atr-scb.js', array(), $this->version, true );
	...
	public function block_tracking_scripts() {
		...
		// Only block if no consent given
		if ( ! $this->has_consent() ) {
			?>
			<script>
			// Enhanced blocking: Prevent tracking scripts from loading AND executing
			(function() {
				'use strict';
				
				// 1. BLOCK SCRIPT LOADING - Prevent tracking scripts from downloading
				// Check if we've already overridden to prevent conflicts
				if (!window._scbCreateElementOverridden) {
					window._scbCreateElementOverridden = true;
					const originalCreateElement = document.createElement;
					// Store original method for restoration
					window._scbOriginalCreateElement = originalCreateElement;
					
					document.createElement = function(tagName) {
						const element = originalCreateElement.call(document, tagName);
						
						if (tagName.toLowerCase() === 'script') {
							// Use a safer approach - don't redefine properties multiple times
							if (!element._scbSrcBlocked) {
								element._scbSrcBlocked = true;
								let originalSrc = '';
								...
								Object.defineProperty(element, 'src', {
									...
									set: function(value) {
										if (value && (
											...
											console.log('SCB: BLOCKED tracking script from loading:', value);
											originalSrc = 'data:text/javascript,console.log("SCB: Script blocked - no consent given")';
										} else {
											originalSrc = value;
										}
									},
				...
				// 2. BLOCK EXISTING SCRIPTS - Disable already loaded tracking scripts
				function blockExistingScripts() {
					const trackingScripts = document.querySelectorAll('script[src*="google-analytics"], script[src*="googletagmanager"], script[src*="gtm.js"], script[src*="facebook"], script[src*="fbevents"]');
					trackingScripts.forEach(script => {
						if (script.src && !script.src.includes('data:text/javascript')) {
							console.log('SCB: Disabling existing tracking script:', script.src);
							// Store original source for restoration
							script.setAttribute('data-original-src', script.src);
							script.src = 'data:text/javascript,console.log("SCB: Existing script disabled - no consent given")';
						}
					});
				}
				
				// 3. BLOCK FUNCTION CALLS - Prevent tracking functions from working
				// Enhanced gtag blocking - handle both new and existing functions
				if (window.gtag) {
					// Store original function if it exists and isn't already our blocked version
					...
					// Create blocked version if gtag doesn't exist
					window.gtag = function() {
						console.log('SCB: gtag() blocked - no consent given');
						return false;
					};
				...
				// Enhanced GA blocking - handle both new and existing functions
				if (window.ga) {
					// Store original function if it exists and isn't already our blocked version
					...
					// Create blocked version if ga doesn't exist
					window.ga = function() {
						console.log('SCB: ga() blocked - no consent given');
						return false;
					};
				...
				if (!window.dataLayer.push || !window.dataLayer.push.toString().includes('SCB: dataLayer.push() blocked')) {
					// Store original function if it exists
					if (window.dataLayer.push && typeof window.dataLayer.push === 'function') {
				...
				// Enhanced Facebook Pixel blocking - handle both new and existing functions
				if (window.fbq) {
					// Store original function if it exists and isn't already our blocked version
					...
					// Create blocked version if fbq doesn't exist
					window.fbq = function() {
						console.log('SCB: fbq() blocked - no consent given');
						return false;
					};
				...
				if (!window._gaq.push || !window._gaq.push.toString().includes('SCB: _gaq.push() blocked')) {
					// Store original function if it exists
					if (window._gaq.push && typeof window._gaq.push === 'function') {
						window._gaq._originalPush = window._gaq.push;
					}
					window._gaq.push = function() {
						console.log('SCB: _gaq.push() blocked - no consent given');
						return false;
					};
				}
				
				// 4. BLOCK IFRAMES - Prevent tracking iframes
				if (!window._scbCreateElementNSOverridden) {
					window._scbCreateElementNSOverridden = true;
					...
					document.createElementNS = function(namespaceURI, qualifiedName) {
						...
						if (qualifiedName.toLowerCase() === 'iframe') {
							if (!element._scbIframeSrcBlocked) {
								element._scbIframeSrcBlocked = true;
								let originalSrc = '';
								...
								Object.defineProperty(element, 'src', {
									...
									set: function(value) {
										if (value && (
											...
											console.log('SCB: BLOCKED tracking iframe:', value);
											originalSrc = 'data:text/html,<div style="padding:20px;text-align:center;color:#666;">SCB: Tracking iframe blocked - no consent given</div>';
										} else {
											originalSrc = value;
										}
									},
				...
				// 5. BLOCK IMG REQUESTS - Prevent tracking pixels
				if (!window._scbImgBlockingOverridden) {
					window._scbImgBlockingOverridden = true;
					const originalCreateElementImg = document.createElement;
					
					document.createElement = function(tagName) {
						const element = originalCreateElementImg.call(document, tagName);
						
						if (tagName.toLowerCase() === 'img') {
							if (!element._scbImgSrcBlocked) {
								element._scbImgSrcBlocked = true;
								let originalSrc = '';
								...
								Object.defineProperty(element, 'src', {
									...
									set: function(value) {
										if (value && (
											...
											console.log('SCB: BLOCKED tracking pixel:', value);
											originalSrc = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
										} else {
											originalSrc = value;
										}
									},
				...
				// 6. BLOCK XMLHTTPREQUEST - Prevent tracking API calls
				if (!window._scbXHROverridden) {
					window._scbXHROverridden = true;
					...
					XMLHttpRequest.prototype.open = function(method, url, async, user, password) {
						if (url && (
							...
							console.log('SCB: BLOCKED tracking API call:', url);
							// Redirect to a harmless URL
							url = 'data:text/plain,SCB: API call blocked - no consent given';
						}
						return originalXHROpen.call(this, method, url, async, user, password);
					};
				}
				
				// 7. BLOCK FETCH - Prevent modern tracking requests
				if (!window._scbFetchOverridden) {
					window._scbFetchOverridden = true;
					...
					window.fetch = function(url, options) {
						if (typeof url === 'string' && (
							...
							console.log('SCB: BLOCKED tracking fetch:', url);
							return Promise.resolve(new Response('SCB: Fetch blocked - no consent given', { status: 403 }));
						}
				...
				// Also block any scripts that might load later
							...
							mutation.addedNodes.forEach(function(node) {
								if (node.nodeType === 1 && node.tagName === 'SCRIPT') {
									if (node.src && (
										...
										console.log('SCB: BLOCKED dynamically added tracking script:', node.src);
										node.src = 'data:text/javascript,console.log("SCB: Dynamic script blocked - no consent given")';
									}
								}
							});
						}
				...
				console.log('SCB: ENHANCED TRACKING BLOCKING ACTIVATED - No consent given');
				console.log('SCB: Blocking: Scripts, Functions, Iframes, Pixels, API calls, Fetch requests');
			})();
			</script>
			<?php
		}
	...
	/**
	 * Inject the cookie consent banner HTML in the footer.
	 *
	 * @since    1.0.0
	 */
	public function inject_banner_html() {
        ...
        <button id="scb-btn-reject" class="scb-btn" type="button">
          <span class="scb-btn-text"><?php echo esc_html( __( 'Reject Non-Essential', 'atr-simple-cookie-consent-banner' ) ); ?></span>
          <span class="scb-btn-loading" style="display: none;"><?php echo esc_html( __( 'Loading...', 'atr-simple-cookie-consent-banner' ) ); ?></span>
        </button>
        <button id="scb-btn-custom" class="scb-btn" type="button"><?php echo esc_html( __( 'Preferences', 'atr-simple-cookie-consent-banner' ) ); ?></button>
				</div>
          ...
          <fieldset>
            ...
            <label><input type="checkbox" name="analytics"> <?php echo esc_html( __( 'Analytics (Google Analytics)', 'atr-simple-cookie-consent-banner' ) ); ?></label><br>
            <label><input type="checkbox" name="marketing"> <?php echo esc_html( __( 'Marketing/Advertising (Facebook/Ads)', 'atr-simple-cookie-consent-banner' ) ); ?></label>
          </fieldset>

						          <div class="scb-actions">
            <button type="submit" class="scb-btn scb-btn-primary">
              <span class="scb-btn-text"><?php echo esc_html( __( 'Save Choices', 'atr-simple-cookie-consent-banner' ) ); ?></span>
              <span class="scb-btn-loading" style="display: none;"><?php echo esc_html( __( 'Loading...', 'atr-simple-cookie-consent-banner' ) ); ?></span>
            </button>
            <button type="button" id="scb-btn-cancel" class="scb-btn"><?php echo esc_html( __( 'Cancel', 'atr-simple-cookie-consent-banner' ) ); ?></button>
          </div>
					</form>
				</div>
				<div class="scb-more" style="display: flex;justify-content: space-between;direction: ltr;">
					        <a href="<?php echo esc_url( get_privacy_policy_url() ?: '#' ); ?>" target="_blank" rel="noopener"><?php echo esc_html( __( 'Privacy Policy', 'atr-simple-cookie-consent-banner' ) ); ?></a>
					<a href="https://atarimtr.co.il/" target="_blank" rel="noopener" role="link">
						<svg inkscape:version="1.2.1 (9c6d41e410, 2022-07-14)" version="1.1" id="svg2" viewBox="0 0 24 24" height="16" width="16" sodipodi:docname="atr-guten-icon.svg" xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape" xmlns:sodipodi="http://www.inkscape.org/namespaces/sodipodi" xmlns="http://www.w3.org/2000/svg" xmlns:svg="http://www.w3.org/2000/svg" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:cc="http://creativecommons.org/ns#" xmlns:dc="http://purl.org/dc/elements/1.1/">
